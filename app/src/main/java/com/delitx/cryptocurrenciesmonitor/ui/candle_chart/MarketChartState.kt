...
class MarketChartState {
    ...
    val transformableState = TransformableState { zoomChange, panChange, _ ->
        ...
        scrollOffset = scrollView(panChange.x)
    }

    val scrollableState = ScrollableState {
        scrollOffset = scrollView(it)
        it
    }
    ...
    private fun scrollView(xChange: Float): Float = if (xChange > 0) {
        (scrollOffset - xChange.scrolledCandles).coerceAtLeast(0f)
    } else {
        (scrollOffset - xChange.scrolledCandles).coerceAtMost(
            (candles.lastIndex - visibleCandles.size).toFloat()
        )
    }

    private fun scaleView(zoomChange: Float) {
        if ((zoomChange < 1f && visibleCandleCount / zoomChange <= candles.size) ||
